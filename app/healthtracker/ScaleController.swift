...
class ScaleController: StepController {
    ...
    func setupSkipButton() {
        ...
        if (survey.steps[currentStep].subtitle != nil) {
            skipButton.frame = CGRect(x: view.frame.width/2 - skipLabelWidth/2, y: 15 + label.frame.size.height + subtitleLabel.frame.size.height + 30 + answerLabel.frame.size.height + 10 + minValueLabel.frame.size.height + 45 + nextButton.frame.size.height + 5, width: skipLabelWidth, height: skipLabelHeight)
        } else {
            skipButton.frame = CGRect(x: view.frame.width/2 - skipLabelWidth/2, y: 15 + label.frame.size.height + 30 + answerLabel.frame.size.height + 10 + minValueLabel.frame.size.height + 45 + nextButton.frame.size.height + 5, width: skipLabelWidth, height: skipLabelHeight)
        }
